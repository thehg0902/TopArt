...
2026-07-06 | polish-round | Operator tweaks: hero text staggered entrance (css-only, motion-safe), buttons radius-sm→radius-lg (softer), mobile nav open animation (panel drop + link cascade), hero videos changed from intro-then-loop-B to alternating ping-pong A→B→A→B with rAF crossfades | human | operator instruction; QA re-ran 0 FAIL
